...
def man()
  ...
  print "    options:\n"
  print "      -n | name [name]  (searches for profiles with the given name)\n"
  print "      -s | skill [skill] (searches for profiles with the given skill)\n"
  print "  remove|rm  [option]... [ID]... removes profile or a job position\n"
  print "    options:\n"
...
def main()
  ...
  while(1)
    ...
    if command =~/add\s+-p/i
      ...
      searchMembers("skill")
      manLevelOneShort()
    elsif command =~/(remove|rm)\s+-j\s+([0-9]+)/i
      if $1=="12345678"
